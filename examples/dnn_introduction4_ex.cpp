...
{
    using namespace std;
    ifstream fin1((folder_name + "/emnist-byclass-train-images-idx3-ubyte").c_str(), ios::binary);
    if (!fin1)
    {
        fin1.open((folder_name + "/emnist-byclass-train-images-idx3-ubyte").c_str(), ios::binary);
    }

    ifstream fin2((folder_name + "/emnist-byclass-train-labels-idx1-ubyte").c_str(), ios::binary);
    if (!fin2)
    {
        fin2.open((folder_name + "/emnist-byclass-train-labels-idx1-ubyte").c_str(), ios::binary);
    }

    ifstream fin3((folder_name + "/emnist-byclass-test-images-idx3-ubyte").c_str(), ios::binary);
    if (!fin3)
    {
        fin3.open((folder_name + "/emnist-byclass-test-images-idx3-ubyte").c_str(), ios::binary);
    }

    ifstream fin4((folder_name + "/emnist-byclass-test-labels-idx1-ubyte").c_str(), ios::binary);
    if (!fin4)
    {
        fin4.open((folder_name + "/emnist-byclass-test-labels-idx1-ubyte").c_str(), ios::binary);
    }

    if (!fin1) throw error("Unable to open file emnist-byclass-train-images-idx3-ubyte");
    if (!fin2) throw error("Unable to open file emnist-byclass-train-labels-idx1-ubyte");
    if (!fin3) throw error("Unable to open file emnist-byclass-test-images-idx3-ubyte");
    if (!fin4) throw error("Unable to open file emnist-byclass-test-labels-idx1-ubyte");
    ...
    if (magic != 2051|| nr != 28 || nc != 28)
        throw error("emnist dat files are corrupted. ");
    ...
    if (!fin1) throw error("Unable to read emnist-byclass-train-images-idx3-ubyte");
    if (!fin2) throw error("Unable to read emnist-byclass-train-labels-idx1-ubyte");
    if (!fin3) throw error("Unable to read emnist-byclass-test-images-idx3-ubyte");
    if (!fin4) throw error("Unable to read emnist-byclass-test-labels-idx1-ubyte");
    ...
    if (fin1.get() != EOF) throw error("Unexpected bytes at end of emnist-byclass-train-images-idx3-ubyte");
    if (fin2.get() != EOF) throw error("Unexpected bytes at end of emnist-byclass-train-labels-idx1-ubyte");
    if (fin3.get() != EOF) throw error("Unexpected bytes at end of emnist-byclass-test-images-idx3-ubyte");
    if (fin4.get() != EOF) throw error("Unexpected bytes at end of emnist-byclass-test-labels-idx1-ubyte");
}
...
int main(int argc, char** argv) try
{
    ...
    if (argc != 2)
    {
        cout << "This example needs the EMNIST dataset to run!" << endl;
        cout << "You can get EMNIST from https://www.westernsydney.edu.au/bens/home/reproducible_research/emnist" << endl;
    ...
    std::vector<matrix<unsigned char>> testing_images;
    std::vector<unsigned long>         testing_labels;
    load_emnist_dataset(argv[1], training_images, training_labels, testing_images, testing_labels);
    ...
    trainer.set_min_learning_rate(0.00000000001);
    ...
    // the file exists.
    trainer.set_synchronization_file("emnist_sync", std::chrono::seconds(20));
    ...
    net.clean();
    serialize("emnist_network_62.dat") << net;
